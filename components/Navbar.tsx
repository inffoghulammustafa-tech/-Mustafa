        ...
        <div className="flex justify-between h-20 items-center">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => handleLinkClick('home')}>
            <div className="w-16 h-12 flex items-center justify-center overflow-hidden">
              <img src="https://rohanilaj.com/wp-content/uploads/2025/01/Rohani-Ilaj-Logo-Design.png" alt="Logo" className="h-full object-contain" />
            </div>
            <div>
          ...
          <div className="hidden lg:flex space-x-6 items-center">
            <button onClick={() => handleLinkClick('home')} className={`font-semibold transition-colors ${currentPage === 'home' ? 'text-emerald-600' : 'text-emerald-900 hover:text-emerald-600'}`}>Home</button>
            ...
            <button onClick={() => handleLinkClick('admin')} className="text-slate-500 hover:text-emerald-600 font-semibold transition-colors text-sm">Admin</button>
            <a href="https://wa.me/923000000000" className="px-6 py-2.5 bg-emerald-700 text-white font-bold rounded-full hover:bg-emerald-800 transition-all shadow-md hover:shadow-lg active:scale-95">Contact Expert</a>
          </div>

          <div className="lg:hidden flex items-center">
            <button onClick={() => setIsOpen(!isOpen)} className="text-emerald-900 p-2 focus:outline-none"><i className={`fa-solid ${isOpen ? 'fa-times' : 'fa-bars'} text-2xl`}></i></button>
          </div>
        </div>
      </div>
      {isOpen && (
          ...
          <div className="px-4 pt-2 pb-8 space-y-1">
            ...
            <div className="py-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {topics.map(topic => (
                  <button key={topic} onClick={() => handleLinkClick(topic)} className="block w-full text-left px-4 py-3 text-sm font-semibold text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 rounded-xl transition-colors border border-gray-50">{topic}</button>
                ))}
              </div>
            </div>
          </div>
